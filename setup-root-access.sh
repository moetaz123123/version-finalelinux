...
# Usage: sudo ./setup-root-access.sh
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo "🔑 Ou connectez-vous en tant que root"
    exit 1
fi
...
for tenant in $TENANTS; do
    ...
    local chroot_dir="/home/chroot_$tenant"
    local project_dir="www.$tenant.localhost/project_$tenant"
