...
# Usage: ./setup-ultra-isolation.sh
...
sudo sed -i '/^# Configuration chroot/,/^$/d' "$SSH_CONFIG"
sudo sed -i '/^Match User /,/^$/d' "$SSH_CONFIG"
...
for tenant in $TENANTS; do
    ...
    sudo mkdir -p "$chroot_dir/home/$tenant/$project_dir"
    
    # Créer le dossier source s'il n'existe pas
    if [ ! -d "/home/$tenant/$project_dir" ]; then
        sudo mkdir -p "/home/$tenant/$project_dir"
    ...
    if [ -d "/home/$tenant/$project_dir" ]; then
        ...
        sudo cp -r "/home/$tenant/$project_dir"/* "$chroot_dir/home/$tenant/$project_dir/" 2>/dev/null || true
    fi
    
    # Créer le fichier de bienvenue
    sudo tee "$chroot_dir/home/$tenant/$project_dir/README.txt" > /dev/null << EOF
...
EOF
    ...
    sudo tee "$chroot_dir/home/$tenant/$project_dir/index.html" > /dev/null << EOF
<!DOCTYPE html>
<html>
<head>
...
EOF
    ...
    sudo chown -R "$tenant:$tenant" "$chroot_dir/home/$tenant"
    sudo chmod -R 755 "$chroot_dir/home/$tenant"
...
EOF
    ...
    # Shell et home
    sudo usermod -s /bin/bash "$tenant" 2>/dev/null || true
    sudo usermod -d "/home/$tenant" "$tenant" 2>/dev/null || true
    
    # Mot de passe
    PASSWORD="${tenant}@2024!"
    if echo "$tenant:$PASSWORD" | sudo chpasswd 2>/dev/null; then
